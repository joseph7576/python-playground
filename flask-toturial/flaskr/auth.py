    ...
    if request.method == 'POST':
        ...
        if error is None:
            try:
                ...
                db.commit()
            except db.IntegrityError: # if the username already exists.
...
def login():
    if request.method == 'POST':
        ...
        if error is None:
            ...
            session['user_id'] = user['id'] #? stores in cookie :D
...
def login_required(view):
    ...
    def wrapped_view(**kwargs):
        if g.user is None:
            return redirect(url_for('auth.login')) #? The name associated with a view is also called the endpoint, and by default it’s the same as the name of the view function.
